...
แต่ EE_Buffer ต้องมีขนาด **1024 bytes** เพื่อครอบคลุม settings layout (0-1023)

| MCU | HW Page Erase | EE_Buffer | EEPROM Address |
|-----|--------------|-----------|---------------|
| CH32V203RBT6 (D8) | 256B (Fast) | 1024 bytes | 0x0800F000 |
| CH32V203C8T6 (D6) | 256B (Fast) | 1024 bytes | 0x0800F000 |
...
// eeprom.c line 50-60
  ...
  #define PAGE_SIZE                  1024   // EE_Buffer + data range
...
```
 ...
 │                 ───                        │
 │ PA0  LED (GPIO output, define LEDBLINK)          │
...
| LED | PA0 | main.c | GPIO output (LedBlink() defined, not yet in main loop) |
