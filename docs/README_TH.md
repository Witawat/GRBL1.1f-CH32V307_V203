# GRBL 1.1f — คู่มือการใช้งาน (ภาษาไทย)
...
| Flash | 128K (V307VCT6) / 168K (V307VET6) |
| RAM | 64K (V307VCT6) / 128K (V307VET6) |
...
│
├── docs/                          # 📄 เอกสาร
│   ├── README_TH.md               #    ไฟล์นี้ (คู่มือการใช้งานภาษาไทย)
│   ├── GRBL_GCODE_REFERENCE.md    #    อ้างอิง G-code และ M-code ทั้งหมด
...
โปรเจคนี้รองรับทั้ง CH32V307 และ CH32V203RBT6 อยู่แล้ว — เพียงเปลี่ยน config ใน `scripts/build.bat`:

```batch
:: เลือก MCU ที่ต้องการ — แค่ comment/uncomment บรรทัดนี้
call "%~dp0mcu_config_CH32V307.bat"
::call "%~dp0mcu_config_CH32V203.bat"
```

**หมายเหตุ**: โฟลเดอร์ `mcu/CH32V203RBT6/` มี V20x SDK, startup, linker script พร้อมใช้งานแล้ว — ดูบทที่ 12
...
CH32V203RBT6 เป็น MCU ที่เล็กกว่าและถูกกว่า CH32V307 — **ขณะนี้โปรเจครองรับทั้งสอง MCU แล้ว**:

| รายการ | CH32V307 (ค่าเริ่มต้น) | CH32V203RBT6 |
|--------|----------------------|--------------|
| Core | QingKe V4F + FPU | QingKe V4B (ไม่มี FPU) |
| Flash | 128K | 128K |
| RAM | 64K | 64K |
| แกน | 6 แกน (X,Y,Z,A,B,C) | 3 แกน (X,Y,Z) |
| แพ็คเกจ | LQFP100 | LQFP64 |
| USB | USB High-Speed | USB Full-Speed (หรือปิดใช้ Serial แทน) |
| ราคา | ~$3-4 | ~$0.50-1 |
| สถานะ | ✅ Build ผ่าน | ✅ Build ผ่าน (config พร้อม) |

### 12.2 ขั้นตอนการย้าย

ดูรายละเอียดทั้งหมดใน `docs/MIGRATION_V203RBT6.md`

**ขั้นตอนย่อ** (ทุกไฟล์เตรียมไว้พร้อมแล้ว):

1. เปลี่ยน `scripts/build.bat`: ใช้ `mcu_config_CH32V203.bat` (comment บรรทัด CH32V307, uncomment บรรทัด CH32V203)
2. ไฟล์ `mcu/CH32V203RBT6/` มี V20x SDK, startup, linker script พร้อมแล้ว
3. ไฟล์ `grbl/cpu_map.h` มี `CH32V203_RBT6_3AXIS` pin map พร้อมแล้ว
4. ไฟล์ `grbl/config.h` มี conditional defines สำหรับ V203 พร้อมแล้ว
5. Build → สำเร็จ
...
> **Maintainer**: GRBL CH32V307/V203 Project Team
