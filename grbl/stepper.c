...

{
  ...
  // Then pulse the stepping pins
    ...
    #else  // Normal operation
      STEP_PORT->OUTDR = ((STEP_PORT->OUTDR & ~STEP_MASK) | st.step_outbits);
...
#endif
	{
		STEP_PORT->OUTDR = ((STEP_PORT->OUTDR & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK));
		TIM3->CTLR1 &= ~TIM_CEN;
		TIM3->INTFR = ~(TIM_UIF | TIM_CC1IF); // clear UIF flag
		TIM3->CNT = 0;
	}
#endif
...
//#endif
  TIM3->INTFR = ~(TIM_UIF | TIM_CC1IF); // clear UIF and CC1IF flags
  STEP_PORT->OUTDR = ((STEP_PORT->OUTDR & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK));
  DIRECTION_PORT->OUTDR = ((DIRECTION_PORT->OUTDR & ~DIRECTION_MASK) | (dir_port_invert_mask & DIRECTION_MASK));
